...
EagerDataFrame from_columns_ordered(
    ...
    for (const auto& [name, arr] : columns) {
        if (!arr) { throw std::runtime_error("from_columns: null array for '" + name + "'"); }
        if (nrows < 0) nrows = arr->length();
        else if (arr->length() != nrows) { throw std::runtime_error("from_columns: column length mismatch");}
...
EagerDataFrame read_csv(const std::string& path) {
    ...
    auto infile = unwrap(arrow::io::ReadableFile::Open(path, pool),"read_csv: open '" + path + "'");
    ...
    auto reader = unwrap(arrow::csv::TableReader::Make( arrow::io::IOContext(pool), infile, read_opts, parse_opts, convert_opts), "read_csv: make reader");
...
#ifdef DFL_HAVE_PARQUET
    auto pool   = arrow::default_memory_pool();
    auto infile = unwrap(arrow::io::ReadableFile::Open(path, pool),"read_parquet: open '" + path + "'");

    auto reader = unwrap(parquet::arrow::OpenFile(infile, pool),"read_parquet: open file reader");
...
#else
    (void)path;
    throw std::runtime_error("read_parquet: DataFrameLib was built without Parquet support");
#endif
}
